...
class LightVLAD():
    ...
    def forward(self,reshaped_input):
        ...
        reshaped_input = tf.reshape(reshaped_input,[-1,int(self.max_frames),int(self.feature_size)])
...
class NetFV():
    ...
    def forward(self,reshaped_input):
        ...
        fv2 = tf.reshape(fv2,[-1,int(self.cluster_size*self.feature_size)])
        ...
        fv1 = tf.reshape(fv1,[-1,int(self.cluster_size*self.feature_size)])
        fv1 = tf.nn.l2_normalize(fv1,1)
...
class NetVLADModelLF(models.BaseModel):
  ...
  def create_model(self,
    ...
    reshaped_input = tf.reshape(model_input, [-1,int( feature_size)])
